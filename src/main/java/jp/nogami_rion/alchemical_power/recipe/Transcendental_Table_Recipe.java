...
public class Transcendental_Table_Recipe implements Recipe<SimpleContainer> {
    private final NonNullList<Ingredient> inputItems;
    private final ItemStack output;
    private final ResourceLocation id;

    public Transcendental_Table_Recipe(NonNullList<Ingredient> inputItems, ItemStack output, ResourceLocation id) {
...
//                ;
    ...
    public static class Type implements RecipeType<Transcendental_Table_Recipe> {
        public static final Type INSTANCE = new Type();
        public static final String ID = "transcendental_table_recipe";
    }

    public static class Serializer implements RecipeSerializer<Transcendental_Table_Recipe> {

        public static final Serializer INSTANCE = new Serializer();
        public static final ResourceLocation ID = new ResourceLocation(Alchemical_power.MODID, "transcendental_table_recipe");

        @Override
        public Transcendental_Table_Recipe fromJson(ResourceLocation resourceLocation, JsonObject jsonObject) {
            ItemStack output = ShapedRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(jsonObject, "output"));

            JsonArray ingredients = GsonHelper.getAsJsonArray(jsonObject, "ingredients");
            NonNullList<Ingredient> inputs = NonNullList.withSize(170, Ingredient.EMPTY);

            for (int i = 0; i < inputs.size(); i++) {
                inputs.set(i, Ingredient.fromJson(ingredients.get(i)));
            }

            return new Transcendental_Table_Recipe(inputs, output, resourceLocation);
        }

        @Override
        public @Nullable Transcendental_Table_Recipe fromNetwork(ResourceLocation resourceLocation, FriendlyByteBuf friendlyByteBuf) {
        ...
        @Override
        public void toNetwork(FriendlyByteBuf friendlyByteBuf, Transcendental_Table_Recipe transcendentalTableRecipe) {
            friendlyByteBuf.writeInt(transcendentalTableRecipe.inputItems.size());

            for (Ingredient ingredient : transcendentalTableRecipe.getIngredients()) {
                {
                    ingredient.toNetwork(friendlyByteBuf);
                }
            }
            friendlyByteBuf.writeItemStack(transcendentalTableRecipe.getResultItem(null), false);
        }
    }
}