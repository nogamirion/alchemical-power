...
import jp.nogami_rion.alchemical_power.block.entity.Transcendental_Table_Entity;
...
public class Transcendental_Table_Menu extends AbstractContainerMenu {
    public final Transcendental_Table_Entity blockEntity;
    private final Level level;
    private final ContainerData data;

    public Transcendental_Table_Menu(int pContainerid, Inventory inv, FriendlyByteBuf extraData){
        this(pContainerid,inv,inv.player.level().getBlockEntity(extraData.readBlockPos()),new SimpleContainerData(171));
    }

    public Transcendental_Table_Menu(int pContainerid, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypes.TRANSCENDENTAL_TABLE_MENU.get(),pContainerid);
        checkContainerSize(inv,41);
        blockEntity = ((Transcendental_Table_Entity) entity);
...
//            this.addSlot(new SlotItemHandler(iItemHandler, 24,82, 96));
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = 171;  // must be the number of slots you have!
    @Override
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level,blockEntity.getBlockPos()),
                player, blocklist.TRANSCENDENTAL_TABLE.get());
    }
    ...
    private void addPlayerHotbar(Inventory playerInventory) {
        for (int i = 0; i < 9; ++i) {
            this.addSlot(new Slot(playerInventory, i, 250 + i * 18, 206));
        }
    }
}
