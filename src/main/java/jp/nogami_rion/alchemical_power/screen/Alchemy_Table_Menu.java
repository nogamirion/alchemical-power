...
public class Alchemy_Table_Menu extends AbstractContainerMenu {
    ...
    public Alchemy_Table_Menu(int pContainerid, Inventory inv, FriendlyByteBuf extraData){
        this(pContainerid,inv,inv.player.level().getBlockEntity(extraData.readBlockPos()),new SimpleContainerData(11));
    }

    public Alchemy_Table_Menu(int pContainerid, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypes.ALCHEMY_TABLE_MENU.get(),pContainerid);
        checkContainerSize(inv,11);
        blockEntity = ((Alchemy_Table_Entity) entity);
